...
def calc_max_per_k_elems(arr: List[int], k: int) -> List[int]:
    ...
    for i in range(k):
        while dq and arr[dq[-1]] < arr[i]: # <-- finding the largest element in first sub array
            dq.pop()
        dq.append(i)
    print(dq)
...
"""
Note: copied from https://github.com/ruppysuppy/Daily-Coding-Problem-Solutions/blob/master/Solutions/018.py
...
if __name__ == '__main__':
    s = time.perf_counter()
    print("Result:", print(calc_max_per_k_elems([10, 5, 2, 7, 8, 7], 3)))
    e = time.perf_counter()
    ...
    s = time.perf_counter()
    print(calc_max_per_k_elems([92, 91, 17, 46, 45, 36, 9], 3))
    e = time.perf_counter()
    print(f"time consumed: {e-s:.6f} seconds")